...
> Hacé esto **después** de tener la URL pública del backend (Railway).
...
| Variable | Valor |
|---|---|
| `VITE_API_URL` | `https://TU-BACKEND.up.railway.app` *(sin `/api` ni barra final)* |
...
2. Volvé a **Railway → backend → Variables** y poné esa URL en `AllowedOrigins__0`
   (ej. `https://chipo-tienda-frontend.vercel.app`). Railway redeploya solo.
...
las instrucciones de DNS. Después actualizás `AllowedOrigins__0` en Railway con el dominio nuevo.
